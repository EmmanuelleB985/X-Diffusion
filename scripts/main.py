...
def random_id(
    digits_nb=4, include_letters=True, only_capital=True, unique_digits=False
):
    """
...
with open("./configs/sd-brats-finetune-c_concat-256.yaml", "r") as stream:
    # Converts yaml document to python object
    config = yaml.safe_load(stream)

api = wandb.Api()
wandb.init(
    name=random_id(), project="mri", config=config, notes="", sync_tensorboard=False
)

Logger = WandbLogger(project="mri", log_model="all")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # default to ddp
        trainer_config["accelerator"] = "ddp"  # "ddp"
