...
def get_3d_locations(d, h, w, device_):
    ...
    # stack locations
    locations_3d = (
        torch.stack([normalised_locs_x, normalised_locs_y, normalised_locs_z], dim=4)
        .view(-1, 3, 1)
        .to(device_)
    )
    return locations_3d
...
def rotate3d_tensor(
    input_tensor, rotation_matrix, mode="nearest", padding_mode="zeros"
):
    """
    ...
    rotated_3d_positions = torch.bmm(
        rotation_matrix.view(1, 3, 3).expand(d * h * w, 3, 3), locations_3d
    ).view(1, d, h, w, 3)
    ...
    grid = torch.stack([rot_locs[0], rot_locs[1], rot_locs[2]], dim=4).view(
        1, d, h, w, 3
    )
    # here we use the destination voxel-positions and sample the input 3d data trilinearly
    rotated_signal = nn.functional.grid_sample(
        input=input_tensor,
        grid=grid.repeat(bs, 1, 1, 1, 1),
        mode=mode,
        align_corners=True,
...
for brain in glob.glob(database + "**/*t2f.nii.gz", recursive=True):
    ...
    mask[
        int(z_offset - IMG.shape[0] // 2) : int(z_offset) + IMG.shape[0] // 2,
        int(x_offset) - IMG.shape[1] // 2 : int(x_offset) + IMG.shape[1] // 2,
        int(y_offset) - IMG.shape[2] // 2 : int(y_offset) + IMG.shape[2] // 2,
    ] = IMG

    centred_IMG = torch.tensor(mask, dtype=torch.float64)
    centred_IMG = centred_IMG.unsqueeze(0).unsqueeze(0)

    axes = [[1, 0, 0], [0, 0, 1], [0, 0, 1]]
    stack_tensors = []
    for axis in axes:
