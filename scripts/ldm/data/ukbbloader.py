...
class UKBBDatasetModuleFromConfig(pl.LightningDataModule):
    def __init__(
        ...
        **kwargs,
        ...
        paths = [
            os.path.join(self.root_dir, name) for name in os.listdir(self.root_dir)
        ]
...
class UKBBDataset(Dataset):
    ...
    def __init__(
        self,
        root_dir="/work/emmanuelle/UKBiobank/Dixon-Mri-v2-stitched/",
        image_transforms=[],
    ...
    def __getitem__(self, idx: int):
        ...
        data["T"] = torch.tensor(
            [
                self.rotation_axis[idx][0][0],
                self.rotation_axis[idx][0][1],
                self.rotation_axis[idx][0][2],
                self.depth[idx],
            ]
        )
...
if __name__ == "__main__":
    ...
    test = paths[math.floor(total_objects / 100.0 * 99) :]  # used last 1% as validation
