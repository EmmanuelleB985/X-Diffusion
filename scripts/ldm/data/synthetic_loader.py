...
class SyntheticDatasetModuleFromConfig(pl.LightningDataModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.val_paths = paths[math.floor(total_objects / 100.0 * 90) :]
        self.train_paths = paths[: math.floor(total_objects / 100.0 * 80)]

    def train_dataloader(self):
        dataset = Synthetic_Dataset(
            root_dir=self.train_paths,
...
class Synthetic_Dataset(Dataset):
    ...
    def __init__(
        ...
        for i in range(self.files):
            ...
            sdata_target = self.tform(
                Image.fromarray(((target_slice * 255)).astype("uint8"))
            )
    ...
    def __getitem__(self, idx: int):
        ...
        data["T"] = torch.tensor(
            [
                self.rotation_axis[idx][0],
                self.rotation_axis[idx][1],
                self.rotation_axis[idx][2],
                self.depth[idx],
            ]
        )  # try with axis of rotation values
...
if __name__ == "__main__":
    ...
    d2 = SyntheticDatasetModuleFromConfig(
        root_dir=1, batch_size=1, total_view=1, train=True
    )
