...
class BratsDatasetModuleFromConfig(pl.LightningDataModule):
    def __init__(
        ...
        **kwargs,
        ...
        ]  # used last 10% as validation
        self.train_paths = paths[
            : math.floor(total_objects / 100.0 * 80.0)
        ]  # used first 80% as training
...
class BratsDataset(Dataset):
        ...
        for s in self.files:
            ...
            for i in range(self.total_view):
                if i > 1:
                    avgdot = torch.einsum(
                        "ij,kj->ik",
                        torch.from_numpy(dat[i, :, :]).float(),
                        torch.from_numpy(dat[i - 1, :, :]).float(),
                    )
            ...
            starget = Image.fromarray(np.uint8(target * 255.0))
    ...
    def __getitem__(self, idx: int):
        ...
        data["T"] = torch.tensor(
            [
                self.rotation_axis[idx][1][0],
                self.rotation_axis[idx][1][1],
                self.rotation_axis[idx][1][2],
                self.depth[idx],
            ]
        )
...
if __name__ == "__main__":
    ...
    d2 = BratsDatasetModuleFromConfig(
        root_dir=path_nifti, batch_size=1, total_view=3, train=True
    )
