...
class DDPM(pl.LightningModule):
    ...
    # The SSIM metric
    # def ssim_metric(self,pred, target, mask=None):
    #    return np.mean(ssim(pred, target, mask=mask, size_average=False))

    # The PSNR metric
    def PSNR(self, pred, target, mask=None, reshape=False):
        b = pred.size(0)
        if not (mask is None):
            b = pred.size(0)
            mse_err = (pred - target).pow(2) * mask
            if reshape:
            ...
            if reshape:
                mse_err = (pred - target).pow(2).reshape(b, -1).mean(dim=1)
            else:
    ...
    # The PSNR metric 3D over a whole volume
    def PSNR3D(self, pred, target, mask=None, reshape=False):
        b = pred.size(0)
        if not (mask is None):
            b = pred.size(0)
            mse_err = (pred - target).pow(2) * mask
    ...
    #    ssim = ssim(img1, img2, mask=mask, size_average=False)
    #    return ssim.mean()
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        psnr = self.PSNR(model_out, target)

        # ssim = self.ssim_metric(model_out, target)

        # loss_dict.update({f'{log_prefix}/SSIM': ssim})

        loss_dict.update({f"{log_prefix}/PSNR": psnr})
...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        if return_first_stage_outputs:
            ...
            for x_sample in xrec:
                ...
                name = "{}_sample_{}.png".format(filename[count], str(count))
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        psnr = self.PSNR(model_output, target)
        loss_dict.update({f"{prefix}/PSNR": psnr})
