...
@torch.no_grad()
def sample_slice_with_denoising(
    input_im,
    T_cond,
    model,
    sampler,
    ...
    def timestep_embedding_with_device(
    ...
    with precision_scope("cuda"):
        with model.ema_scope():
            ...
            if scale != 1.0:
                uc = {}
                uc["c_concat"] = [
                    torch.zeros(n_samples, 4, h // 8, w // 8, device=device)
                ]
...
def save_slice_as_png(tensor_slice, save_path):
    ...
    if tensor_slice.shape[0] in [1, 3]:  # channels first
        img_array = 255.0 * rearrange(tensor_slice.cpu().numpy(), "c h w -> h w c")
    else:  # channels last or single channel
        img_array = 255.0 * tensor_slice.cpu().numpy()
...
def process_batch_to_3d_volume(
    dataloader,
    model,
    sampler,
    output_dir,
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):
        ...
        if save_individual_slices:
            ...
            save_slice_as_png(
                input_normalized[0], os.path.join(input_dir, f"{filename[0]}_input.png")
            )
            save_slice_as_png(
                target_normalized[0],
    ...
    if len(reconstructed_slices) > 0:
        ...
        nifti_path = os.path.join(
            output_dir, f"{patient_name}_reconstructed_volume.nii.gz"
        )
        nib.save(nifti_img, nifti_path)
    ...
    if input_volume.ndim == 4:
        ...
        input_volume = (
            input_slices[0].cpu().numpy()
            if torch.is_tensor(input_slices[0])
            else input_slices[0]
        )
        # Get the actual shape
        if input_volume.shape[0] == 3:  # [C, H, W]
            input_volume = np.stack(
                [s[0, :, :] if s.shape[0] == 3 else s[:, :, 0] for s in input_slices],
                axis=0,
            )
        else:  # [H, W, C]
    ...
    if target_volume.ndim == 4:
        ...
        target_volume = np.stack(
            [s[0, :, :] if s.shape[0] == 3 else s[:, :, 0] for s in target_slices],
            axis=0,
        )
    ...
    nib.save(
        nifti_img, os.path.join(output_dir, f"{patient_name}_target_volume.nii.gz")
    )
...
def main(
    ...
    test_config = {"validation": False, "image_transforms": {"size": image_size}}
        ...
        if isinstance(cond, dict):
            ...
            for key, value in cond.items():
                if isinstance(value, list):
                    fixed_cond[key] = [
                        v.to(model_device) if torch.is_tensor(v) else v for v in value
                    ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="BraTS Diffusion Model Inference - Generate 3D Volumes"
    )

    # Model arguments
    parser.add_argument(
        "--device_idx", type=int, default=0, help="GPU device index (default: 0)"
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        default="./checkpoints/epoch=000833.ckpt",
        help="Path to model checkpoint",
    )
    parser.add_argument(
        "--config",
        type=str,
        default="./configs/sd-brats-finetune-c_concat-256.yaml",
    ...
    parser.add_argument(
        "--no_generate_denoised",
        dest="generate_denoised",
        action="store_false",
